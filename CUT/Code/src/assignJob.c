...
//initializing mutex lock and time variables
...
//    int add=0; removing warning
    ...
    while (1)
    {
        ...
        {
            cnt=cnt+1;
            int st = stringValid(str);     //checking if string is valid.
    ...
    for(temp=job1->head;temp!=job1->tail->next;temp=temp->next)
                {
                   if(strcmp((*temp).m,m1)==0)     //checking for machine 1
                        {
                   ...
                   if(strcmp((*temp).m,m2)==0)     //checking for machine 2
                        {
                   ...
                   if(strcmp((*temp).m,m3)==0)     //checking for machine 3
                        {
